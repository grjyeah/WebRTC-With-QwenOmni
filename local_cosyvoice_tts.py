...
"""
Local CosyVoice TTS implementation using downloaded model files
"""
...
class LocalCosyVoiceTTS:
    def __init__(self, model_path: str = None, voice: str = "中文女"):
        ...
        # 使用本地模型路径
        self.model_path = model_path or os.path.expanduser("~/.cache/modelscope/hub/models/iic/CosyVoice2-0.5B")
    ...
    def _load_model(self):
        """加载本地CosyVoice模型"""
        try:
            if self.modelscope_available and os.path.exists(self.model_path):
                self.logger.info(f"Loading CosyVoice model from local path: {self.model_path}")
                # 尝试加载本地模型
                self.model = AutoModel.from_pretrained(self.model_path)
                self.logger.info("Successfully loaded local CosyVoice model")
            else:
                self.logger.warning("ModelScope not available or local model path not found")
        except Exception as e:
            self.logger.error(f"Failed to load local CosyVoice model: {e}")
            self.model = None
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Local CosyVoice TTS Demo")
    ...
    try:
        # 创建本地CosyVoice TTS实例
        tts = LocalCosyVoiceTTS(model_path=args.model_path, voice=args.voice)
